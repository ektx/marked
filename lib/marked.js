 ...
 * Copyright (c) 2011-2020, ektx. (MIT Licensed)
...
(function (global, factory) {
  ...
  function splitCells(tableRow, count) {
    ...
    for (var i = 0; i < tableRow.length; i++) {
      var val = tableRow[i]; // 当字符串为 | 且堆中没有特殊符号引用时 且 没有半角开合的符号

      if (val === '|' && !stacks.length && !isOpen) {
        // 追加内容 从完整的内容是截取内容
        cells.push(tableRow.slice(start, i).trim()); // 更新起始位置
  ...
  function () {
    ...
    _proto.link = function link(href, title, text) {
      ...
      var out = '<a target="_blank" href="' + escape$2(href) + '"';
    ...
    _proto.TOC = function TOC(list) {
      var _this = this;

      var out = '';
      var level = 0;
      list.forEach(function (head) {
        var slugger = new Slugger_1();
        var slug = _this.options.headerPrefix + slugger.slug(head.text);
  ...
  function () {
    ...
    _proto.tok = function tok() {
      ...
      switch (this.token.type) {
        case 'TOC':
          {
            var headings = this.allTokens.filter(function (token) {
